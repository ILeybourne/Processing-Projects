...
void setup()
{
  size(500, 500,  three_d ? OPENGL : P2D);
...
void draw()
{
  ...
  waveLengthFactor = Math.ceil(frameNr*waveIncrease*100)/0.5;
  doInterference();
  ...
  for (int i = 0; i < p.size(); i++)
  {
    ...
    if (photo)
    {
      if (pp.z > 0) stroke(clr, 0, 0);
      if (pp.z < 0) stroke(0, 0, clr);
    }
    else
    {
      if (pp.z > 0) stroke(clr, 0, 0);
  ...
  if (photo)
  {
    //saveFrame("Exp2d/"+Math.floor(A*100)/100+"_"+Math.floor(k*100)/100+"/photo/"+frameNr+"22-"+waveLengthFactor+".png");
    photo = false;
    frameNr++;
  }
